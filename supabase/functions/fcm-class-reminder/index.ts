  ...
  const resp = await fetch(`https://fcm.googleapis.com/v1/projects/${projectId}/messages:send`, {
    ...
    body: JSON.stringify({
      message: {
        ...
        data: { title, body, event: "class_reminder" },
        webpush: {
...
async function sendWebPushToSubscription(sub: AnyObj, title: string, body: string) {
  try {
    await webpush.sendNotification(
      ...
      JSON.stringify({ title, body, event: "class_reminder" }),
...
Deno.serve(async () => {
  ...
  const [{ data: defs }, { data: tcRows }, { data: profiles }] = await Promise.all([
  ...
  const dueClasses: Array<{ className: string; startHm: string; diffMin: number }> = [];
  (defs || []).forEach((d: AnyObj) => {
    ...
    if (!cls || sm < 0) return;
    const diff = sm - nowMin;
    // Cron chạy theo chu kỳ + có thể trễ vài phút; mở rộng cửa sổ để không bỏ lỡ thông báo thật.
    if (diff >= 20 && diff <= 29) dueClasses.push({ className: cls, startHm: hm(sm), diffMin: diff });
  });
  ...
  for (const c of dueClasses) {
    ...
    for (const uid of receivers) {
      const title = "🔔 NHẮC HẸN LỚP HỌC";
      const body = `Lớp ${c.className} sẽ bắt đầu sau khoảng ${c.diffMin} phút nữa (lúc ${c.startHm}). Thầy/Cô vui lòng chuẩn bị vào lớp.`;
