-- =============================================================================
-- 057 - Standard exam upload filenames
--
-- Upload files to Storage bucket `exam-files` using ASCII-safe names. The web
-- portal derives the Vietnamese display title from these parts:
--
--   Main exam:  De 001 Vao 10 Thanh Hoa 2025.pdf
--   Answer:     Dap an De 001 Vao 10 Thanh Hoa 2025.pdf
--   Audio:      Audio De 001 Vao 10 Thanh Hoa 2025.mp3
--
-- THPT files use the same form:
--
--   De 001 THPT Thanh Hoa 2025.pdf
--   Dap an De 001 THPT Thanh Hoa 2025.pdf
--
-- Folder names are ignored for parsing, so both of these are equivalent:
...
begin
  ...
  if s ~ '^(dap an|answer|answers|loi giai)\s+' then
    return 'answer';
  end if;
  if s ~ '^(audio|listening|nghe)\s+' then
    return 'audio';
  end if;
  if s ~ '(chuyen de|topic)' then
    return 'topic';
  end if;
  return 'exam';
end;
...
begin
  s := public.exam_file_normalized_text(p_path);
  s := regexp_replace(s, '^(dap an|answer|answers|loi giai)\s+', '', 'i');
