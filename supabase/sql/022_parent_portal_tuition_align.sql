-- =============================================================================
-- 022 — Parent portal: align resolve_class_parent_payment với thu học phí
-- hiện tại (student_tuition_by_class + fn_pending_sessions_for_class, class_names).
-- Chạy trên Supabase SQL Editor. Không đổi bảng / webhook; chỉ thay RPC.
-- =============================================================================
...
begin
  ...
  if v_scope = 'center' then
    ...
    -- Link theo một lớp: chỉ tính đúng lớp trên token
    v_pend := coalesce(public.fn_pending_sessions_for_class(v_student.id, v_link.class_name), 0);
    ...
    select coalesce(stc.charged_sessions, 0) into v_charged
    from public.student_tuition_by_class stc
    where stc.student_id = v_student.id
      and public.normalize_lookup_text(coalesce(stc.class_name, '')) =
        public.normalize_lookup_text(v_link.class_name)
    ...
    where public.normalize_lookup_text(coalesce(cf.class_name, '')) =
      public.normalize_lookup_text(v_link.class_name)
    limit 1;
    v_amount := greatest(0, v_pending * coalesce(v_fee, 0));
  ...
  return jsonb_build_object(
    ...
    'unclassified_present_sessions', v_unclassified,
    'class_options', case when v_scope = 'center' then v_class_options else '[]'::jsonb end,
